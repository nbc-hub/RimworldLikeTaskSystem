...
public class GatherStone : GAction {

    public PigChef pigChef;    
    public override bool PrePerform() {
        // Get a free iron
        
        target = GWorld.Instance.GetQueue("stones").RemoveResource();
    ...
    public override bool PostPerform() {
        ...
        GWorld.Instance.GetWorld().ModifyState("StoneGathering", 1);
        // Give back the iron
        GWorld.Instance.GetQueue("stones").AddResource(target);
        // Remove the iron from the list
        inventory.RemoveItem(target);
        // Give the iron back to the world
        GWorld.Instance.GetWorld().ModifyState("FreeStone", 1);
